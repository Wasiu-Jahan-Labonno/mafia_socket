...
function setupSocketServer(httpServer) {
  ...
  io.on("connection", async (socket) => {
    ...
    socket.on("gangMegJoin", async (data) => {
      try {
        const { gid } = data; // Get gid from data

        if (!gid) {
          return socket.emit("error", { message: "GID is required" });
        }

        let condition = "WHERE gid = ?";
        let params = [gid];

        let gangMembers = await sqlUtil.find("gang_members", condition, params);

        console.log(
          "📜 Gang Members Retrieved:",
          JSON.stringify(gangMembers, null, 2)
        );

        socket.emit("gangMembersList", { members: gangMembers });
      } catch (error) {
        console.error("❌ Error fetching gang members:", error);
        socket.emit("error", { message: "Failed to retrieve gang members." });
      }
    });
